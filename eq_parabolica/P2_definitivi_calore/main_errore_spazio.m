...
% nu = 1;
% u_esatta = @(x,y,t) sin(pi*x) + cos(pi*y) + t.^2; 
% f = @(x,y,t) 1 - (-pi^2 * sin(pi*x) -pi^2 * cos(pi*y));
% u0 = @(x,y) sin(pi*x) + cos(pi*y);
% gD = @(x,y,t) 0;
% gN =  @(x,y,t) [pi * cos(pi*x); -pi * sin(pi * y)];

nu = 1;
u_esatta = @(x,y,t) exp(x) + exp(y) + t.^2; 
f = @(x,y,t) 2*t - exp(x) - exp(y);
u0 = @(x,y) exp(x) + exp(y);
gD = @(x,y,t) exp(x) + exp(y) + t.^2;
